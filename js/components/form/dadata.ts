...
document.addEventListener(
    ...
    () => {
        ...
        inputs.forEach((input, index) => {
            ...
            const dataJson = input.getAttribute('data-json');
            ...
            if (dataJson) {
                const autoCompleteJS = new autoComplete({
                    ...
                    data: {
                        src: async (query) => {
                            try {
                                const url = `${dataJson}?query=${query}`;
                        ...
                        element: (list, data) => {
                            if (!data.results.length) {
                                ...
                                data.results.forEach((item, index) => {
                                    li[index].innerHTML = item.value.city_name;
                                    li[index].setAttribute(
                                        'data-id',
                                        item.value.id
                                    );
                                    li[index].setAttribute(
                                    ...
                                    li[index].classList.add('js-region-save');
                                });
                            }
                        },
                        noResults: true,
                    ...
                    events: {
                        input: {
                            selection: (event) => {
                                ...
                                input.value = selection.city_name;
                                input.dispatchEvent(
